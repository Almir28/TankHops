...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    private func spawnEnemyTank() {
        ...
        // Корпус врага (более агресивный дизайн)
        let bodyPath = UIBezierPath()
        ...
        // Добавляем пеиодическую стрельбу
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if let playerTank = playerTank {
            let tankFrame = CGRect(
                ...
                y: playerTank.position.y - touchAreaSize/2,
                width: touchAreaSize,
                height: touchAreaSize
            )
            
            if tankFrame.contains(location) {
                isTouchingTank = true
                shoot()
            } else {
                isMoving = true
    ...
    // Добавим новый метод для обрабоки столкновения с врагом
    private func handlePlayerEnemyCollision() {
        playerLives -= 1
    }
    
    // Обновим обработку столкновений для босс-танков
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        // Проверяем достижение счета для босса
        checkLevelProgress()
    }
    ...
    private func restartGame() {
        // Удаляем все элементы Game Over экрана
        if let gameOverNode = gameOverLabel {
            gameOverNode.removeFromParent()
            gameOverLabel = nil
        }
        
        if let restartNode = restartButton {
            restartNode.removeFromParent()
            restartButton = nil
        }
        
        // Очищаем сцену от игровых объектов
    ...
    override func didFinishUpdate() {
        ...
        // Очищаем мссв врагов от удалённых нодов
